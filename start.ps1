...
if (Test-Path $envFile) {
    ...
    if ($match) {
        $frontendPort = $match.Matches[0].Groups[1].Value.Trim()
    }
}

# Docker
Write-Host "[1/3] Starting databases (Docker)..."
$dockerProc = Start-Process -FilePath "docker" -ArgumentList "compose up -d" -WorkingDirectory $root -Wait -PassThru -WindowStyle Hidden
...
$backendCmd = "cd /d `"$root\backend`" && call `"$root\backend\venv\Scripts\activate`" && set PYTHONIOENCODING=utf-8 && uvicorn app.main:app --reload >> `"$backendLog`" 2>&1"
